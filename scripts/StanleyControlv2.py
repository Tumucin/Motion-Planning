...
class FindCoeffClass(object):
    ...
    def calculate_coeff(self):
        ...
        print("convert_theta:",convert_Theta)
        print("LineHeading:",LineHeadingError)
        self.headingErr=LineHeadingError-convert_Theta
        
        if abs(self.headingErr) > 2.5:
            if convert_Theta >0 and LineHeadingError <0:
                self.headingErr = math.pi-convert_Theta-LineHeadingError
            if convert_Theta < 0 and LineHeadingError > 0:
                self.headingErr = LineHeadingError -convert_Theta-math.pi
            if convert_Theta > 0 and LineHeadingError < 0 and deriv_y < 0:
                self.headingErr = -LineHeadingError - convert_Theta

        print("self.heading:",self.headingErr)
            

        if abs(self.headingErr) < 0.0001:
            self.headingErr=0
...
if __name__ == "__main__":
    try:
        rospy.init_node('StanleyControlv2',anonymous=True)
